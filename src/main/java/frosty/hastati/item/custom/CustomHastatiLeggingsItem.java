...
import frosty.hastati.item.client.CustomLeggingsRenderer;
...
public class CustomHastatiLeggingsItem extends ArmorItem implements GeoItem {
    private final AnimatableInstanceCache cache = GeckoLibUtil.createInstanceCache(this);

    public CustomHastatiLeggingsItem(RegistryEntry<ArmorMaterial> material, Type type, Settings settings) {
    ...
    public void createGeoRenderer(Consumer<GeoRenderProvider> consumer) {
        consumer.accept(new GeoRenderProvider() {
            private CustomLeggingsRenderer renderer;
            ...
            public @Nullable <T extends LivingEntity> BipedEntityModel<T> getGeoArmorRenderer(T livingEntity, ItemStack itemStack, EquipmentSlot equipmentSlot, BipedEntityModel<T> original) {
                if (this.renderer == null) {
                    this.renderer = new CustomLeggingsRenderer();
                }
